...
async function generateSectionWithRetry(
  ...
  // AI에 실제 데이터를 구체적으로 주입 (섹션별 HIRA 필터링 적용)
  const dataContext = buildDetailedDataContext(hiraContextStr, clinicalTrialsData, drugName, indication, globalData, section.id)
...
/**
 * 섹션별 HIRA 컨텍스트 필터링
 * - epidemiology / patient-segmentation-rwd: 전체 데이터 (성별+연령+지역+기관)
 * - market-segmentation: 지역+기관 데이터만
 * - 그 외 모든 섹션: 핵심 요약만 (성별/연령/지역 상세 분포 제외)
 */
function filterHiraContextForSection(hiraContextStr: string, sectionId: string): string {
  if (!hiraContextStr) return ''

  const FULL_SECTIONS = ['epidemiology', 'patient-segmentation-rwd']
  const REGION_ONLY_SECTIONS = ['market-segmentation', 'regional-analysis']

  if (FULL_SECTIONS.includes(sectionId)) {
    return hiraContextStr // 역학 섹션만 전체 HIRA 데이터 수신
  }

  // 성별 분포 항목이 시작되는 지점 찾기 → 그 앞까지만 사용
  const demographicMarkers = ['■ 성별 환자 분포', '■ 연령대별 환자 분포', '■ 의료기관종별']
  let cutIndex = hiraContextStr.length

  if (REGION_ONLY_SECTIONS.includes(sectionId)) {
    // 지역/기관 데이터는 유지, 성별/연령만 제거
    const ageMarker = hiraContextStr.indexOf('■ 연령대별 환자 분포')
    const genderMarker = hiraContextStr.indexOf('■ 성별 환자 분포')
    // 지역/기관 이전 항목(성별+연령)만 잘라낸 뒤 지역부터 다시 붙이기
    const regionMarker = hiraContextStr.indexOf('■ 의료기관종별')
    if (regionMarker > 0) {
      const summaryEnd = Math.min(
        genderMarker > 0 ? genderMarker : hiraContextStr.length,
        ageMarker > 0 ? ageMarker : hiraContextStr.length
      )
      const summaryPart = hiraContextStr.substring(0, summaryEnd).trimEnd()
      const regionPart = hiraContextStr.substring(regionMarker)
      return summaryPart + '\n\n' + regionPart +
        '\n\n⚠ 연령별/성별 분포는 역학(Epidemiology) 섹션 전담 — 이 섹션에서 반복 금지'
    }
  }

  // 기본: 요약 수치만 (환자수, 급여비, 시장규모, 내원일수)
  for (const marker of demographicMarkers) {
    const idx = hiraContextStr.indexOf(marker)
    if (idx > 0 && idx < cutIndex) cutIndex = idx
  }
  const summaryOnly = hiraContextStr.substring(0, cutIndex).trimEnd()
  return summaryOnly +
    '\n\n⚠ 연령별·성별·지역별 상세 분포는 역학(Epidemiology) 섹션이 전담합니다.' +
    '\n  이 섹션에서 역학 통계를 반복하거나 언급하지 마세요.' +
    '\n  인구 고령화를 성장 동인으로 반복 기술하는 것도 금지입니다.' +
    '\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
}
...
function buildDetailedDataContext(
  ...
  indication: string,
  globalData?: any,
  sectionId?: string  // ← 섹션별 HIRA 필터링 적용
): string {
  const parts: string[] = []

  if (hiraContextStr) {
    // 섹션 ID가 있으면 해당 섹션에 필요한 데이터만 주입
    const filteredHira = sectionId
      ? filterHiraContextForSection(hiraContextStr, sectionId)
      : hiraContextStr
    parts.push(filteredHira)
  }
