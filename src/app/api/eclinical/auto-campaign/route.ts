  ...
  try {
    ...
    const campaign = await prisma.campaign.create({
      data: {
        ...
        status: 'DRAFT',
        channelType: 'SMS',
        objective: `${study.indication} 활자 대상 ${study.phase || ''} 임상시험 참여자 모집. 현재 ${study.currentEnrollment}/${study.targetEnrollment}명 등록 완료, 추가 ${remaining}명 모집 필요.`,
        targetCount: remaining,
        contentTitle: `${study.indication} 임상시험 참여 안내`,
        contentBody: `안녕하세요, {이름}님.\n\n현재 ${study.sponsorName || ''}에서 진행 중인 "${study.title}" 임상시험에 찴여하실 수 있습니다.\n\n■ 대상: ${study.indication} 환자\n■ 연구 단계: ${study.phase || '-'}\n■ 찴여 기관: ${siteNames}\n■ 프로토콜: ${study.protocolNumber}\n\n참여를 원하시면 가까운 참여 기관에 문의해 주세요.\n\n감사합니다.\n그린리본 eClinical`,
        createdById: user.id,
